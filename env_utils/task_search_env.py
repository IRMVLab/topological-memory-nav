...
class SearchEnv(RLEnv):
    ...
    def __init__(self, config: Config, dataset: Optional[Dataset] = None):
        ...
        print(self.habitat_env._task.sensor_suite.observation_spaces.spaces,"jaskdjkasdj")
        obs_dict = {

                'panoramic_rgb':Box(low=0,high=255, shape=(144,192,3), dtype=np.float32),  # self.habitat_env._task.sensor_suite.observation_spaces.spaces['panoramic_rgb'],
                'panoramic_depth': Box(low=0,high=1., shape=(144,192,1), dtype=np.float32),#self.habitat_env._task.sensor_suite.observation_spaces.spaces['panoramic_depth'],
    ...
    def set_random_goals(self):
        ...
        while True:
           ...
           # print(same_floor,"same_floor",goal_pose,self.initial_pose)
    ...
    @property
    def curr_goal(self):
        #print(self.current_episode.goals,"goaaaals")
    ...
    def process_obs(self, obs):
        ...
        T = q.as_rotation_matrix(np.array(q.from_float_array(obs['rotation'])))
        T = np.concatenate([T, obs['position'].reshape(3, 1)], axis=1)
        T = np.concatenate([T, np.array([[0, 0, 0, 1]])], axis=0)
        tar = np.concatenate([obs['target_pose'], [1]], axis=-1)

        local_to_tar = np.matmul(np.linalg.inv(T), tar.reshape(4, 1))
        # print(local_to_tar, "tarrrrrrrrrrr")
        local_to_tar = local_to_tar.reshape(4)[[0, 2]]
        # print(local_to_tar,"asdwdeqwe")
        if local_to_tar[0] != 0 and local_to_tar[1] != 0:
            local_to_tar /= np.linalg.norm(local_to_tar, ord=2)
            delta_totar = np.arctan2(local_to_tar[0], -local_to_tar[1]) / np.pi
        else:
            delta_totar = 0.0
        #obs['delta_totar']=delta_totar
    ...
    def step(self, action):
        ...
        if self.obs['distance']<1 or self.end_time>10:
            action=0
        self._previous_action = action
        #print(self.action_dict,"acacacac")
    ...
    def get_progress_reward(self, observations):
        ...
        reward += max(self.move,0.0) * 0.2#0.2
        ...
        if self.time_t>0 and self._env.get_metrics()['collisions']['is_collision']:
            reward-=0.003
    ...
    def get_done(self, observations):
        ...
        if self.stuck > 50 or diff_floor or self.time_t>300:
            done = True
    ...
    def current_rotation(self):
       # print(self.habitat_env.sim.get_agent_state().rotation,"!23123123")
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if done:
            ...
            if obs['rotation'][0]*obs['rotation'][2]>0:
              rot=q.as_euler_angles(np.array(q.from_float_array(obs['rotation'])))
            else:
                rot=-q.as_euler_angles(np.array(q.from_float_array(obs['rotation'])))
            print(rot,"rot")
            toc = time.time()
