...
class PositionEncoding(nn.Module):
    def __init__(self, n_filters=256, max_len=2000):
        """
...
class Perception(nn.Module):
    def __init__(self,cfg, embedding_network):
        ...
        if self.pe_method == 'embedding':
            ...
            print(cfg.features.visual_feature_dim,"memmeme")
        else:
        ...
        print(feature_dim,memory_dim,cfg.features.visual_feature_dim,"featuredim")
        ...
        ####new
        self.feature_embedding_cur = nn.Sequential(nn.Linear(feature_dim + cfg.features.visual_feature_dim, memory_dim),
                                               nn.ReLU(),
                                               nn.Linear(memory_dim, memory_dim))
        self.global_GCN_cur=GCN(input_dim=memory_dim, output_dim=memory_dim)
        #####
        print(cfg.transformer.hidden_dim,cfg.transformer.nheads,
                                     cfg.transformer.dim_feedforward,"fgggg")
    ...
    def forward(self, observations, mode='train', return_features=False): # without memory
        ...
        #print(global_memory[:,:max_node_num].shape,goal_embedding.unsqueeze(1).repeat(1,max_node_num,1).shape,"goooooal")
        ...
        #####new
        global_memory_with_cur=self.feature_embedding_cur(torch.cat((global_memory[:,:max_node_num], curr_embedding.unsqueeze(1).repeat(1,max_node_num,1)),-1))
        global_context_cur=self.global_GCN_cur(global_memory_with_cur,global_A)
        global_context_cur=self.time_embedding(global_context_cur,relative_time)
        goal_context, goal_attn = self.goal_Decoder(goal_embedding.unsqueeze(1), global_context_cur, global_mask)
        #####
