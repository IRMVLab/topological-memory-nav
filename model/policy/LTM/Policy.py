...
class LTMNet(nn.Module):
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks, mode='', return_features=False):
        ...
        cur1= self.visual_encoder(curr_tensor).view(curr_tensor.shape[0], -1)
        observations['curr_embedding']=self.linearnet(cur1)
        #print(observations['curr_embedding'].shape,"shhhhhhhhhhhhhhhhhhha")
        goal_tensor = observations['target_goal'][:,:,:,:].permute(0, 3, 1, 2)
        goal1=self.visual_encoder(goal_tensor).view(goal_tensor.shape[0], -1)
        observations['goal_embedding']=self.linearnet(goal1)
