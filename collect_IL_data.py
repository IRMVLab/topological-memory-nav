...
parser.add_argument('--ep-per-env', type=int, default=100, help='number of episodes per environments')
...
parser.add_argument('--data-dir', type=str, default="./IL_data_newnewadd", help='directory to save the collected data')
...
def main():
    ...
    # test_list = ['Haxtun', 'Eagerville', 'Denmark', 'Hambleton', 'Angiola', 'Roane', 'Superior', 'Oyens', 'Delton',
    #              'Roxboro',
    #              'Kerrtown', 'Pettigrew', 'Annawan', 'Dunmor', 'Sumas', 'Rosenberg', 'Dryville', 'Rosser', 'Ribera',
    #              'Woonsocket', 'Mobridge','Beach',
    #            'Bolton','Nicut','Cantwell','Matoaca','Rabbit','Quantico']
    # train_list = ['Albertville','Adrian','Ballou','Colebrook','Espanola','Mesic','Nemacolin','plessis','Springhill','Micanopy']#'Spencerville', 'Shelbiana', 'Sawpit', 'Nicut', 'Mosinee', 'Hometown', 'Avonia']
    # print(scenes)
    for space_id, space in enumerate(scenes):
        # if space not in train_list :
        #     continue
        try:
            print('=' * 50)
