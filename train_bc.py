...
parser.add_argument('--data-dir', default='daaaaaa', type=str)
...
def train():
    ...
    observation_space = SpaceDict({
        'panoramic_rgb': Box(low=0, high=256, shape=(144, 192, 3), dtype=np.float32),
        'panoramic_depth': Box(low=0, high=256, shape=(144,192, 1), dtype=np.float32),
        'target_goal': Box(low=0, high=256, shape=(144, 192, 3), dtype=np.float32),
    ...
    config.TORCH_GPU_ID = "0"#args.gpu
    config.freeze()
