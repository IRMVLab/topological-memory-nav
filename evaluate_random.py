...
parser.add_argument("--num-episodes", type=int, default=420)
...
print(torch.cuda.device_count(),"ddddddddddddddddddddevice")
...
def eval_config(args):
    ...
    config.TASK_CONFIG.DATASET.CONTENT_SCENES = ['Spencerville', 'Shelbiana', 'Sawpit', 'Nicut', 'Mosinee', 'Hometown','Avonia']
    ...
    config.TASK_CONFIG.ENVIRONMENT.ITERATOR_OPTIONS.MAX_SCENE_REPEAT_EPISODES = 60#ep_per_env
    if args.stop:
        config.ACTION_DIM = 4
...
def evaluate(eval_config, ckpt):
    ...
    eval_config.noisy_actuation = False#true
    eval_config.features.visual_feature_dim=256
    eval_config.transformer.dim_feedforward =512
    eval_config.transformer.hidden_dim=256
    eval_config.POLICY="LTMPolicy"
    eval_config.freeze()

    print("evaaassssssssssssssssssssssssssssssssssssssssssssssssssssssssssssaal",eval_config.features.visual_feature_dim)
    print(eval_config,"runeee")
    ...
    print(runner.need_env_wrapper,"sdweq")
    ...
    with torch.no_grad():
        ...
        for episode_id in range(args.num_episodes):
           ...
           # print(obs)
            ...
            end_time=0
            while True:
                #env.get_best_action() - 1
                action = runner.step(obs, reward, done, info, env)
               
                obs, reward, done, info = env.step(action)

                dist_togoal.append(obs['distance'][0])
                # print(dist_togoal,"dist_togoal")
                now_position = obs['position']
                # print(now_position,last_position,"1dsadadas")
                delta_dist = torch.norm(now_position[0][[0, 2]] - last_position[0][[0, 2]]).item()
                ...
                if done: break
            spl = info['spl']
            closed_dist = min(dist_togoal).item()
            if np.isnan(spl):
                spl = 0.0
            ...
            if closed_dist<1.0:
              jsondata['if success']=1.0
            else:
                jsondata['if success'] = 0.0
            #jsondata['spl']=info['spl']
            jsondata['pathdist']=total_dist
            jsondata['closed_dist']=closed_dist
            jsondata['optimal dist']=env.habitat_env._sim.geodesic_distance(env.habitat_env.current_episode.goals[0].position,
                                                   env.current_episode.start_position)
            jsondata['dist']=np.linalg.norm(np.array(env.habitat_env.current_episode.goals[0].position)[[0,2]]-np.array(env.current_episode.start_position)[[0,2]])
            jsondata['enddist']=obs['distance'][0].item()
            strai_or_cur='straight'  if  jsondata['optimal dist']<1.2*jsondata['dist'] else 'curve'
            if jsondata['optimal dist']<3:
                mode='easy'
            elif jsondata['optimal dist']<5:
                mode='medium'
            else:
                mode='hard'
            jsondata['label']=[strai_or_cur,mode]
            # with open("./testjson_newnotmft/"+scene_name+'.json',"a",encoding='utf-8') as fw:
